...
describe Facter::CacheManager do
  ...
  let(:seached_core_fact) do
    instance_spy(Facter::SearchedFact, name: 'os', fact_class: instance_double(Facts::Debian::Os::Name),
  ...
  let(:seached_facts) { [seached_core_fact, searched_custom_fact] }
  ...
  let(:fact_groups) { instance_double(Facter::FactGroups) }
  ...
  describe '#resolve_facts' do
    context 'with no cache dir' do
      ...
      it 'returns searched facts' do
        sf, _cf = cache_manager.resolve_facts(seached_facts)
        expect(sf).to eq(seached_facts)
      end

      it 'returns no cached facts' do
        _, cf = cache_manager.resolve_facts(seached_facts)
    ...
    context 'with cached facts' do
      before do
        ...
        allow(File).to receive(:read).with(cache_file_name).and_return(cached_core_fact)
      end

      it 'returns cached fact' do
        allow(fact_groups).to receive(:get_group_ttls).with(group_name).and_return(1000)

        _, cached_facts = cache_manager.resolve_facts(seached_facts)
      ...
      it 'deletes cache file' do
        ...
        cache_manager.resolve_facts(seached_facts)
