...
describe Facter::FactsUtils::Converter do
  subject(:converter) { Facter::FactsUtils::Converter }

  describe '#to_mb' do
    it 'converts bytes to mega bytes' do
      expect(converter.to_mb(256_586_343)).to eq(244.7)
    end

    it 'returns nil if value is nil' do
      expect(converter.to_mb(nil)).to be(nil)
    end

    it 'returns nil if value is string' do
      expect(converter.to_mb('2343455')).to be(nil)
    end

    it 'returns 0 if value is 0' do
      expect(converter.to_mb(0)).to eq(0.0)
    end
  end

  describe '#to_hz' do
    it 'returns nil if value is 0' do
      expect(converter.to_hz(0)).to be_nil
    end

    it 'returns nil if value is string' do
      expect(converter.to_hz('test')).to be_nil
    end

    it 'converts to GHz' do
      expect(converter.to_hz(2_300_000_000)).to eql('2.30 GHz')
    end

    it 'converts to Hz even if argument is string' do
      expect(converter.to_hz('2400')).to eql('2.40 kHz')
    end
  end
